...
frappe.ui.form.on("Cheque Payment", {
setup(frm) {
...
// Outgoing cheques: only Payable-side vouchers make sense per party type.
const PARTY_VOUCHER_MAP = {
Supplier: { "Purchase Invoice": "supplier", "Journal Entry": "party" },
Employee: { "Expense Claim": "employee", "Journal Entry": "party" },
Customer: { "Journal Entry": "party" },
Shareholder: { "Journal Entry": "party" },
};

frm.set_query("doc_type", "cheque_payment_items", () => ({
filters: { name: ["in", Object.keys(PARTY_VOUCHER_MAP[frm.doc.party_type] || {})] },
}));

frm.set_query("deduction_account", "cheque_payment_items", () => ({
filters: {
is_group: 0,
disabled: 0,
...
frm.set_query("voucher_no", "cheque_payment_items", (doc, cdt, cdn) => {
...
.call({
method:
"treasury.treasury.doctype.cheque_payment.cheque_payment.get_preview_ledger",
args: {
company: frm.doc.company,
...
items: JSON.stringify(frm.doc.cheque_payment_items || []),
...
frm.clear_table("cheque_payment_items");
} else {
...
.call({
method:
"treasury.treasury.doctype.cheque_payment.cheque_payment.get_party_details",
args: {
party_type: frm.doc.party_type,
party: frm.doc.party,
},
...
let alloc = 0;
(frm.doc.cheque_payment_items || []).forEach((r) => {
...
frappe.ui.form.on("Cheque Payment Items", {
...
frappe.call({
method:
"treasury.treasury.doctype.cheque_payment.cheque_payment.get_voucher_summary",
...
// Payment Entry style: default the allocation to the voucher outstanding
