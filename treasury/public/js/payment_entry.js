...
function _recalc_if_multi(frm) {
    if (_treasury_feature_enabled(frm) && frm.doc.multi_expense) _recalc_total(frm);
}
...
function _recalc_total(frm) {
    ...
    const currency = _bank_currency(frm) || frm.doc.treasury_expense_currency;
    ...
    if (
        ...
        frm.doc.treasury_expense_currency === currency
    ) {
        return;
    }
    ...
    frm.doc.treasury_expense_currency = currency;
    frm.dirty();
    for (const f of ["paid_amount", "received_amount", "treasury_total_amount",
                     "base_paid_amount", "base_received_amount", "treasury_expense_items"]) {
        frm.refresh_field(f);
    }
}
